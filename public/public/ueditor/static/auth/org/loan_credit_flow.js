loan.creditFlow = loan.creditFlow || {
	...
	view : function(menuId,rowid){
		var rowData = $("#loanCreditGridId").jqGrid('getRowData',rowid);
	...
	histQuery : function(){
		...
		$("#loanCreditGridId").jqGrid("setGridParam", {   
			postData:{'format':'json','userName':userName,'menuId':$('#menuId').val(),'startDate':startDate,'endDate':endDate},
			page:1
	    }).trigger('reloadGrid');
	...
	query : function(){
		...
		$("#loanCreditGridId").jqGrid("setGridParam", {   
			postData:{'searchOrgId':searchOrgId,'userName':userName},
			page:1
	    }).trigger('reloadGrid');

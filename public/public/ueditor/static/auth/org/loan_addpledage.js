loan.addpledage = loan.addpledage || {
	menuId:'-1',//菜单号
	url:loan.basePath+"/guarantee/addPledage",//提交的url
	...
	userDetail : function(userId) {
		...
		if(_userId) {
			userId = _userId;
			parent.loan.tabcut.childAddIframe("客户详情",loan.basePath+"/customer/toCusDetail?menuId="+loan.addpledage.menuId+"&userId="+userId,"customerDetail",true);
		} else {
	 ...
	 */
	submitAdd:function() {
		loan.addpledage.url = loan.basePath+"/guarantee/addPledage";
		...
		if(!ownerIdKind) {
			loan.tooltip("请选择所有权人证件类型","error");
			return;
		}
		
		if(!warrantIdKind) {
			loan.tooltip("请选择抵押物证件类型","error");
			return;
		}
		...
		if(namount > new Number(eamount)) {
			loan.tooltip("抵押金额不可以超过评估金额","error");
			return;
		}
		...
		//校验必传文件
		var fileList;
		if(!loan.addpledage.validateFiles()){
			return;
		} else {
			fileList = loan.addpledage.parseFileJson();
		}
		
		var params = {
				'menuId':loan.addpledage.menuId,
				'menuLog':'1',
				'userId':userId,
				'ownerName':ownerName,
		...
		if(loan.addpledage.fileUpdate){
			$.artDialog({
				title:"提示",
				content:"您有已选择但未上传的文件，现在保存将不会上传这些文件，按确定继续保存，按取消终止保存？",
				ok:function(){
					loan.addpledage.doSubmit(params);
				},
				cancel:function(){
					return false;
				}
	 ...
	 */
	submitEdit:function() {
		loan.addpledage.url = loan.basePath+"/guarantee/updatePledage";
		var userId = $("#userId").val();
		if(!userId) {
			loan.tooltip("请选择用户","error");
			return;
	...
	upload:function(rowid) {
		...
		$("#loanUpload_"+rowid).upload({
			...
			// 其他表单数据
			params: {"menuId":loan.addpledage.menuId},
			dataType: 'text',
	...
	validateFiles:function() {
		...
		loan.addpledage.fileUpdate = false;
		for(var i = 0; i < datas.length; i++) {
			...
			if(fileValue) {
				loan.addpledage.fileUpdate = true;
			}
		}
		return true;
	},
	...
	queryWarrantInfo:function(warrantId){
		loan.ajaxDo({
            ...
            params:{ 'menuId':loan.addpledage.menuId,'format':'json','warrantId':warrantId},
            successCallback:function(result){
        		...
        		if(domain) {
        			...
        			$("#count").val(domain.count);
        			$("#unit").val(domain.unit);
        			$("#usedYears").val(domain.usedYears);
        			$("#address").val(domain.address);
	...
	initAttachPage:function() {
		jQuery("#attachGridId").jqGrid({
			...
			colModel: [
	           ...
	           {name:'select',index:'select',sortable: false, align:'center',width:270,title:false,
					formatter:function(cellvalue, options, rowObject){
						...
						"<input type='button' class='uploadclass' onclick='loan.addpledage.upload("+options.rowId+")' value='上传' />"+
	  	...
	  	//设置附件条目
		loan.addpledage.queryAttachItem();
	  	//更新页面设置
		loan.addpledage.initUpdateData();
	},
	/***
	 * 查询附件条目
	 */
	queryAttachItem:function(){
		var length = loan.addpledage.menuId.length;
		if(length > 2) {
			length = length -2;
		}
		var functionId = loan.addpledage.menuId.substr(0,length);
		loan.ajaxDo({
			url: loan.basePath+ "/attach/queryAttach",
			params:{format:'json','menuId':loan.addpledage.menuId,'status':'0','functionId':functionId,'page':'1','rows':'9999'},
            successCallback:loan.addpledage.attachItemCallBack,
            successTip:false, //提示
            bizErrTip:false,  //提示
            chainPar:this
        });
	...
	queryAttachInfo:function(WarrantId){
		loan.ajaxDo({
			...
			params:{format:'json','menuId':loan.addpledage.menuId,"checkId":WarrantId},//'menuId':'7004'这个后面加上
            successCallback:function(result){
	...
	initPageType:function() {
		...
		if(menuId){
			loan.addpledage.menuId = menuId;
		}
		$("#ownerIdKind").unbind("change").bind("change",function(){
			loan.addpledage.personIdCheck("ownerIdKind", "ownerIdNo");
		});
	...
	initUpdateData:function(){
		...
		if(id) {
			...
			loan.addpledage.queryWarrantInfo(id);
			loan.addpledage.queryAttachInfo(id);
	...
	getBusiDate:function(menuId) {
		...
		// 		loan.addpledage.busiDate = new Date(date.substr(0,4),date.substr(4,2)-1,date.substr(6,2));
		// 	},
		// 	successTip:false, //提示
		// 	bizErrTip:false,  //提示
		// 	chainPar:this
...
$(function(){
	...
	$(".openw").click(function(){
		loan.s_addPop("选择用户",loan.basePath+"/common/selectloan?status=userName&menuId=600801&submitFunc=submit2warrant&closeFunc=close2warrant","",680,380,{isChangeClose:true});
	});
});

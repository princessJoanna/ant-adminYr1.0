loan.mortgage = loan.mortgage || {
	...
	add : function(menuId,rowid) {
		parent.loan.tabcut.childAddIframe("新增质押物",loan.basePath+"/guarantee/toAddMortgage?opType=add&menuId="+menuId,"AddMortgage",true,true);
	},
	...
	edit : function(menuId,rowid) {
		...
		if(status != "0") {
			loan.tooltip("只有状态为未登记的质押物可以修改","error");
			return;
		}
		//获取修改的id
		var id = $("#warrantGridId").jqGrid('getCell',rowid,'warrantId');
		parent.loan.tabcut.childAddIframe("修改质押物",loan.basePath+"/guarantee/toAddMortgage?opType=up&menuId="+menuId+"&warrantId="+id,"edittoMortgage",true,true);
	},
	...
	del : function(menuId,rowid) {
		...
		var status = $("#warrantGridId").jqGrid('getCell',rowid,'status');
		if(status != "0") {
			loan.tooltip("只有状态为未登记的质押物可以删除","error");
			return;
		}
		//确认是否要删除
		$.artDialog({
	...
	lookInfo:function(menuId,rowid){
		$("#rowid").val(rowid);
		var id = $("#warrantGridId").jqGrid('getCell',rowid,'warrantId');
		loan.s_addPop("查看详情",loan.basePath+"/guarantee/toMortgageInfo?warrantId="+id+"&menuId="+menuId,"",680,450,{isRefresh:true,isChangeClose:true});	
	},

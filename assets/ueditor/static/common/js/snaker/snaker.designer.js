(function(snaker){
    ...
    designer.rect=function(o, r){
        ...
        _text.click(function(){
            if (!designer.config.editable) {
                ...
                //var win = window.open(designer.config.ctxPath + '/config/form?lookup=1' , "窗口" , "width=" + iWidth + ", height=" + iHeight + ",top=" + iTop + ",left=" + iLeft + ",toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no,alwaysRaised=yes,depended=yes");
        ...
        this.toBeforeXml=function(){
            ...
            for(var k in _o.props){
                if(k=="name"&&_o.props[k].value==""){
                    ...
                    loan.tooltip(_o.type+" 名称 不能为空","error");
                    return ""
                }
    ...
    designer.init=function(c, o){
        designer.config.rects={};
        designer.config.paths={};
        var _w = o.width||snaker(window).width(), _h = o.height||snaker(window).height(), _r = Raphael(c,_w*1.5,_h*1.5), _states = designer.config.rects, _paths = designer.config.paths;
            ...
            snaker("#save").click(function(){
                ...
                for(var c in designer.config.props.props){
                    if((c=="name"||c=="displayName")&&designer.config.props.props[c].value==""){
                        //alert("流程定义名称、显示名称、实例启动Url不能为空");
                        loan.tooltip("流程定义名称、显示名称、实例启动Url不能为空","error");
                        return;
                    }
                    // 2016-06-12 将name的值赋值给隐藏域
                    var proccessName = designer.config.props.props['name'].value;
                    $('#processName').val(proccessName);
                ...
                for(var node in _states){
                    if(_states[node]){
                    	if(_states[node].getProps().assignee){
                    		var assigneeVal = _states[node].getProps().assignee.value;
                    		if(assigneeVal == ""){
                    			var disName = _states[node].getProps().displayName.value != "" ? "【" + _states[node].getProps().displayName.value + "】" : _states[node].getProps().name.value;
                    			loan.tooltip(disName + "没有选择参与者！","error");
                                return;
                    		}
                    	}
                ...
                for(var idx=0;idx<tarray.length;idx++){
                    if (nary[idx]==nary[idx+1]){
                        ...
                        loan.tooltip("连接线名称不能重复[" + nary[idx] + "]","error");
                        return;
                    }
                }

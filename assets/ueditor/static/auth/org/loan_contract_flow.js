loan.contractFlow = loan.contractFlow || {
	init : function(){
	...
	view : function(menuId,rowid){
		var rowData = $("#loanContractGridId").jqGrid('getRowData',rowid);
		parent.loan.tabcut.childAddIframe("查看审批",loan.basePath + "/process/display?processId=" + rowData.processId + "&orderId=" + rowData.orderId + "&menuId=310408","start",true,true);
	},
	viewHist : function(menuId,rowid){
		var rowData = $("#loanContractGridId").jqGrid('getRowData',rowid);
		parent.loan.tabcut.childAddIframe("查看审批",loan.basePath + "/process/displayHist?processId=" + rowData.processId + "&orderId=" + rowData.orderId + "&menuId=310408","start",true,true);
	},
	// 处理
	exec : function(menuId,rowid){
		var rowData = $("#loanContractGridId").jqGrid('getRowData',rowid);
		parent.loan.tabcut.childAddIframe("任务处理",loan.basePath + "/flow/all" + "?menuId="+menuId+"&processId=" + rowData.processId + "&taskId=" + rowData.taskId + "&orderId=" + rowData.orderId ,"execute",true,true);
	},
	// 历史记录查询
	histQuery : function(){
		...
		$("#loanContractGridId").jqGrid("setGridParam", {   
